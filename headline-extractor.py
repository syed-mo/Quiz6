...
bitarray_len = 8*len(understood_words)
bitarray_bf = bitarray(bitarray_len)
bitarray_bf.setall(0)

def add_item(bitarray,word):
    for i in range(4):
        ind = mmh3.hash(word,i*23)%bitarray_len
        bitarray[ind] = 1

for word in understood_words:
    add_item(bitarray_bf,word)

with open('bloom.txt','wb') as bloom:
    bitarray_bf.tofile(bloom)
